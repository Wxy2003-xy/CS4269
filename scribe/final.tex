...
\subsection{The Resolution Proof System for First-Order Logic}The resolution proof system is a refutation-based logic used for automated theorem proving. It generalizes propositional resolution to First-Order Logic (FOL) by incorporating unification. Instead of proving that a formula $\phi$ is valid, we show that $\neg \phi$ is unsatisfiable.\subsubsection{The Resolution Rule and Unification}In FOL, resolution operates on clauses in Skolem Normal Form (SNF). Given two clauses with no variables in common:$$ C_1 = \{L_1, \dots, L_k\} \quad \text{and} \quad C_2 = \{M_1, \dots, M_n\} $$If there exist literals $L_i \in C_1$ and $M_j \in C_2$ such that $L_i$ and $\neg M_j$ are unifiable with a Most General Unifier (MGU) $\sigma$, the \textbf{resolvent} is:$$ \text{Res}(C_1, C_2) = (C_1 \setminus \{L_i\})\sigma \cup (C_2 \setminus \{M_j\})\sigma $$\subsubsection{Soundness of FOL Resolution}\begin{theorem}[Soundness]If there exists a resolution derivation of the empty clause $\square$ from a set of clauses $\mathcal{C}$, then $\mathcal{C}$ is unsatisfiable.\end{theorem}\begin{proof}[Proof Sketch]
The proof is based on the fact that the resolution rule is truth-preserving under any interpretation:
\begin{enumerate}
\item Let $\mathfrak{A}$ be a model of $\mathcal{C}$.
\item If $\mathfrak{A} \models C_1$ and $\mathfrak{A} \models C_2$, then for any substitution $\sigma$, $\mathfrak{A} \models C_1\sigma$ and $\mathfrak{A} \models C_2\sigma$.
\item Since $L_i\sigma$ and $M_j\sigma$ are complementary literals, any assignment that satisfies both clauses must satisfy the remaining literals in the resolvent.
\item By induction, if $\square$ (the empty clause, which is always false) is derived, no model $\mathfrak{A}$ can exist.
\end{enumerate}
\end{proof}\subsubsection{Completeness of FOL Resolution}\begin{theorem}[Refutational Completeness]If a set of clauses $\mathcal{C}$ is unsatisfiable, then there exists a resolution derivation of the empty clause $\square$.\end{theorem}\begin{proof}[Proof Sketch]The proof typically utilizes Herbrand's Theorem and the Lifting Lemma:\begin{enumerate}\item \textbf{Herbrand's Theorem:} If $\mathcal{C}$ is unsatisfiable, there exists a finite set of ground instances $\mathcal{C}_{ground}$ that is unsatisfiable in propositional logic.\item \textbf{Propositional Completeness:} By the completeness of propositional resolution, there is a derivation of $\square$ from $\mathcal{C}_{ground}$.\item \textbf{Lifting Lemma:} For every resolution step between ground instances $C_1', C_2'$ at the propositional level, there exists a corresponding resolution step between the general clauses $C_1, C_2$ at the FOL level using unification.\item Therefore, the propositional derivation of $\square$ can be "lifted" to a first-order derivation of $\square$.\end{enumerate}\end{proof}
...
The Nelson-Oppen method addresses the problem: given two theories $T_1$ and $T_2$, how can we decide the satisfiability of a formula $\phi$ in the combined theory $T_1 \cup T_2$?\begin{definition}[Convex Theory]A theory $T$ is \textbf{convex} if for any conjunction of literals $\Gamma$, if $\Gamma \vDash_T \bigvee_{i=1}^n (x_i = y_i)$, then there exists some $j \in \{1, \dots, n\}$ such that $\Gamma \vDash_T (x_j = y_j)$.\end{definition}\textbf{Prerequisites for Nelson-Oppen:}\begin{enumerate}\item The signatures of the theories must be disjoint ($\Sigma_1 \cap \Sigma_2 = \emptyset$), except for the equality symbol ($=$).\item The theories must be \textbf{stably infinite} (every satisfiable formula has a model with an infinite universe).\item The formulas must be quantifier-free conjunctions of literals.\end{enumerate}
\subsubsection{The Nelson-Oppen Algorithm}The procedure involves two main phases: Purification and Equality Propagation.\textbf{Step 1: Purification}The input formula $\phi$ is transformed into an equivalent formula $\phi_1 \land \phi_2$ such that each $\phi_i$ contains only symbols from $\Sigma_i$.\begin{itemize}\item This is achieved by introducing auxiliary variables to replace "alien" terms.\item For example, in $f(g(x)) = x$ where $f \in \Sigma_1$ and $g \in \Sigma_2$, we replace $g(x)$ with a new variable $v$, resulting in $f(v) = x \land v = g(x)$.\end{itemize}\textbf{Step 2: Equality Propagation}The individual theory solvers communicate by sharing information about equalities between shared variables (variables that appear in both $\phi_1$ and $\phi_2$).\begin{itemize}\item Solver $D_1$ checks if $\phi_1$ is satisfiable in $T_1$.\item Solver $D_2$ checks if $\phi_2$ is satisfiable in $T_2$.\item If either is unsatisfiable, the combined formula is unsatisfiable.\item If $D_1$ implies an equality between shared variables $x = y$, this equality is added to $\phi_2$.\item If $D_2$ implies an equality, it is added to $\phi_1$.\item This continues until a contradiction is found or no more equalities can be inferred.\end{itemize}\subsection{Complexity and Completeness}\begin{itemize}\item \textbf{Completeness:} For convex theories (like the theory of equality or linear arithmetic over reals), propagating single equalities is sufficient. For non-convex theories (like linear arithmetic over integers), the solvers may need to perform case splits on disjunctions of equalities, leading to higher complexity.\item \textbf{Decidability:} If the individual satisfiability problems for $T_1$ and $T_2$ are decidable, then the Nelson-Oppen procedure provides a decision procedure for the combined theory $T_1 \cup T_2$.\end{itemize}
