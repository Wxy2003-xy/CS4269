...
\end{proof}
Now prove that $\forall i, \Gamma_i$ is finitely satisfiable. 
\end{proof}
...
\begin{enumerate}
  \item $|\phi_G|$ is polynomial in $k+|V|+|E|$,
  \item $\phi_G$ is satisfiable if and only if $G$ has a vertex cover of size at most $k$.
\end{enumerate}
...
\section*{Q9}
(Extra Credit) Encode the Independent Set problem as propositional logic.
\begin{enumerate}
  \item Construct a formula $\phi_{G,k}$ satisfiable iff $G$ has an independent set of size $k$.
  \item Write $\phi_{G,3}$ explicitly for a given graph on five vertices.
\end{enumerate}

\section*{Q10}
(Extra Credit) Implement a solver for Independent Set using a SAT solver as an oracle.
The program should read a graph and $k$, construct $\phi_{G,k}$, and determine satisfiability.
...
\section*{Q11}
Prove that if $\Gamma$ is satisfiable, then $\Gamma$ is finitely satisfiable.
...
\begin{enumerate}
  ...
  \item Give examples as requested in the handout.
\end{enumerate}
...
\section*{Q14}
Let $P=\{p_1,p_2,\dots\}$ and define $\Gamma_0,\Gamma_1,\dots$ as in the handout.
Prove that $\Delta=\bigcup_{i\ge0}\Gamma_i$ is finitely satisfiable.

\section*{Q15}
Prove that if $\Gamma$ is finitely satisfiable, then $\Gamma$ is satisfiable.
