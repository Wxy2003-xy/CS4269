...
\begin{definition}{Valuation of terms}
    \begin{align*}
        ...
        &Val_{\mathfrak{A}, \alpha}(x) = \alpha(c) \in U\\
...
\begin{definition}{Valuation of Formula}
    ...
    \begin{enumerate}
        ...
        \[\mathfrak{A}, \alpha \models \phi \iff \text{ there is } u \in U, \mathfrak{A}, \alpha[x\mapsto u] \models \phi\]
        \item $\phi \equiv \forall x, \varphi$
        \[\mathfrak{A}, \alpha \models \phi \iff \text{ for all } u \in U, \mathfrak{A}, \alpha[x\mapsto u] \models \phi\]
    \end{enumerate}
\end{definition}
...
Now we have defined machineries needed to prove Compactness for FOL, let's outline the overall proof structure. 
From our definition of grounded terms, it obvious that we are trying to remove the elements that give FOL more expressiveness
over propositional logics, such as variables and quantifiers, and we are encapsulating relation into the concept of ground atom,
It becomes clear that the goal is to build some connection between FOL and PL, and use certain proven properties of PL to establish 
properties we want to prove for FOL. 
