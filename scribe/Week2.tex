...
\title{Week 2-4: Propositional Logic (HW2 start from page 29)}
...
\begin{proof}
    ...
    \begin{itemize}
        \item Case 1: $\phi_{i+1}$ is in $\Gamma$ or $\phi_{i+1}$ is from one of the axioms, trivial
        \item Case 2: $\phi_{i+1}$ is obtained from MP, where $\exists j \leq i, \phi_j $
    \end{itemize}
\end{proof}
...
\end{proof}
\begin{enumerate}
    \item Use deduction and proof by contradiction to move everything to the assumpton side, and have $\bot$ on the conclusion side
    \begin{align*}
        &\Gamma \cup \{A\} \vdash B \implies \Gamma \vdash A \rightarrow B\\
        &\Gamma \cup \{\neg A\} \vdash \bot \implies \Gamma \vdash A
    \end{align*}
    We can prove $\Gamma \cup \{A, \neg B\} \vdash \bot$ to prove $\Gamma \vdash A \rightarrow B$,
\end{enumerate}
...
\begin{theorem}[Soundness]
    ...
    \[\vdash_{RES} \Gamma \implies \Gamma \text{ is unsatisfiable}\]
\end{theorem}
\begin{proof}
    $\vdash_{RES} \Gamma \rightarrow$ there exists a finite sequence ends with $C_k$
\end{proof}
\begin{theorem}[Completeness]
The resolution proof system is complete
\[\Gamma \text{ is unsatisfiable } \implies \vdash_{RES} \Gamma\]
...
\section*{Theorem 1 (Deduction Theorem)}
...
\section*{Question 2}
Prove Theorem 1. Do not use the completeness theorem (Theorem 3) to establish this! We will instead use Theorem 1 to establish Theorem 3 in later questions. \textit{Hint: One direction is easier. For the other direction, consider a proof $\pi = \psi_1, \psi_2, \dots, \psi_n$ of $\psi$ from $\Gamma \cup \{\varphi\}$, 
Use induction on the index $i$ to show that the formula $(\varphi \rightarrow \psi_i)$ can be proved from $\Gamma$}

\section*{Question 3}
Prove the following (in the Hilbert proof system). Here, $\varphi, \psi,$ and $\rho$ are propositional logic formulae, and $\Gamma$ is a set of propositional logic formulae
\begin{itemize}
    \item (a) $\vdash \varphi \rightarrow \varphi$ 
    \item (b) $\vdash (\psi \rightarrow \varphi) \rightarrow \varphi$, where $\psi = \varphi \rightarrow \perp$
    \item (c) $\{\perp\} \vdash \varphi$ 
    \item (d) $\vdash \psi \rightarrow (\varphi \rightarrow \rho)$ where $\psi = \varphi \rightarrow \perp$
\end{itemize}
...
\section*{Question 13}
Inductively show that $\Delta_i$ is unsatisfiable, for every $0 \le i \le k$. Also show that $\Delta_i \neq \emptyset$ for every $0 \le i \le k$
