...
\end{theorem}
\begin{proof}
\textbf{Base Case:} Let $\alpha = p$ for some proposition $p \in \mathcal{P}$.
\begin{itemize}
    \item $s(p) = 1$ and $c(p) = 0$.
    \item Since $1 = 0 + 1$, $P(p)$ holds for all atomic propositions.
\end{itemize}

\textbf{Inductive Case 1 (Negation):} Assume $P(\psi)$ holds, i.e., $s(\psi) = c(\psi) + 1$. Let $\alpha = (\neg \psi)$.
\begin{itemize}
...
\textbf{Inductive Case 2 (Binary Connectives):} Assume $P(\psi_1)$ and $P(\psi_2)$ hold. Let $\alpha = (\psi_1 c \psi_2)$ where $c \in \{\lor, \to\}$.
\begin{itemize}
