...
\begin{lemma}[Relevance Lemma for FOL]
    Let $\varphi$ be a FOL formula, 
    \[(\forall x \in FVar(\varphi), \alpha_1(x) = \alpha_2(a)) \implies (\mathfrak{A}, \alpha_1 \models \varphi \iff \mathfrak{A}, \alpha_2 \models \varphi)\]
\end{lemma}
\begin{lemma}
...
When there is no quantifier prefix, $\phi \equiv \psi \implies \phi $ is satisfiable $\iff \psi$ is satisfiable.
\subsubsection*{Inductive steps:}
Inductive hypothesis: $P(\pi, \psi, u): (\pi, \psi)$ is equisatisfiable with $Skol(\pi, \psi, u)$
\begin{itemize}
    \item Case 1: leading existential, no processed universals
    \[\exists x \chi \rightarrow \chi[x \mapsto c_x]\]
    
    \item Case 2: leading existential, exist processed universals
    \[\]
    \item Case 3: leading universal
    \[\forall x \chi \rightarrow \forall x Skol(\chi)\]
\end{itemize} 
...
\\
\mathrm{Gr}(\Gamma) \text{ is satisfiable}
...
\begin{definition}[Propositional satisfiable]
...
\subsubsection{$\Gamma$ is satisfiable $\implies Gr(\Gamma)$ is satisfiable}
...
\end{theorem}
\begin{proof}
\subsubsection*{Base Case:} Let $\alpha = p$ for some proposition $p \in \mathcal{P}$.
\begin{itemize}
    \item $s(p) = 1$ and $c(p) = 0$.
    \item Since $1 = 0 + 1$, $P(p)$ holds for all atomic propositions.
\end{itemize}

\subsubsection*{Inductive Case 1 (Negation):} Suppose $P(\psi)$ holds, i.e., $s(\psi) = c(\psi) + 1$. Let $\alpha = (\neg \psi)$.
\begin{itemize}
    \item $s(\neg \psi) = s(\psi)$ no new sentence symbols are added.
    \item $c(\neg \psi) = c(\psi)$ $\neg$ not binary connective.
    \item Thus, $s(\alpha) = s(\psi) = c(\psi) + 1 = c(\alpha) + 1$. $P(\alpha)$ holds.
\end{itemize}

\subsubsection*{Inductive Case 2 (Binary Connectives):} Suppose $P(\psi_1)$ and $P(\psi_2)$ hold. Let $\alpha = (\psi_1 c \psi_2)$ where $c \in \{\lor, \to\}$.
\begin{itemize}
    \item $s(\alpha) = s(\psi_1) + s(\psi_2)$ (sum of occurrences in sub-formulae).
    \item $c(\alpha) = c(\psi_1) + c(\psi_2) + 1$ (sum of binary connectives plus the new $\star$).
    \item By the inductive hypothesis: $s(\alpha) = (c(\psi_1) + 1) + (c(\psi_2) + 1)$.
    \item Rearranging gives $s(\alpha) = (c(\psi_1) + c(\psi_2) + 1) + 1$.
    \item Therefore, $s(\alpha) = c(\alpha) + 1$. $P(\alpha)$ holds.
\end{itemize}

\end{proof}
\subsection*{1.2}

We define the property $P(\alpha)$ as follows: the length of $\alpha$, denoted $L(\alpha)$, 
is of the form $4k + 1$ and the number of sentence symbols, $s(\alpha)$, is $k+1$ for some $k \in \mathbb{N}$

\begin{proof}
\subsubsection*{Base Case:} Let $\alpha = p$ for some proposition $p \in \mathcal{P}$
\begin{itemize}
    \item The length $L(p) = 1 = 4\cdot 0 + 1$
    \item The number of sentence symbols $s(p) = 1$
    \item Thus, $P(p)$ holds.
\end{itemize}

\subsubsection*{Inductive Case:} Suppose $P(\psi_1)$ and $P(\psi_2)$ hold
Then $L(\psi_1) = 4k_1 + 1$ and $s(\psi_1) = k_1 + 1$.
Also $L(\psi_2) = 4k_2 + 1$ and $s(\psi_2) = k_2 + 1$.
Let $\alpha = (\psi_1 c \psi_2)$ where $c$ is a binary connective

\subsubsection*{Length of $\alpha$:}
\begin{itemize}
    \item The string $\alpha$ consists of: one left parenthesis, $\psi_1$, one binary connective, $\psi_2$, and one right parenthesis
    \item $L(\alpha) = 1 + L(\psi_1) + 1 + L(\psi_2) + 1$.
    \item $L(\alpha) = 3 + (4k_1 + 1) + (4k_2 + 1) = 4k_1 + 4k_2 + 5$.
    \item $L(\alpha) = 4(k_1 + k_2 + 1) + 1$.
    \item Let $k = k_1 + k_2 + 1$, we see $L(\alpha) = 4k + 1$, odd
\end{itemize}

\subsubsection*{Number of sentence symbols:}
\begin{itemize}
    \item $s(\alpha) = s(\psi_1) + s(\psi_2) = (k_1 + 1) + (k_2 + 1) = k_1 + k_2 + 2$.
    \item Using $k = k_1 + k_2 + 1$ from above, $s(\alpha) = (k_1 + k_2 + 1) + 1 = k + 1$
    \item $k + 1 = \frac{4k + 4}{4} > \frac{4k + 1}{4}$
\end{itemize}
...
\begin{proof}
    ...
    and denote the logically equivalent formula using the later connector set from $\phi$ as $\psi$
    Define property $P(\phi)$ as follows: there exists a logically equivalant formula over $P$ using the connector set $\{\bot, \rightarrow\}$.
    \subsubsection*{Base Case:}
    \begin{itemize}
        \item $\forall \phi = p \in P, \psi = p$
    \end{itemize}
    \subsubsection*{Inductive Case 1:} Suppose $P(\phi)$ for some formula, $\exists \psi \equiv \phi$
    \begin{itemize}
        \item $(\neg \phi) = (\neg \phi) \lor ((\neg \phi) \lor (\neg \phi))) = \phi \rightarrow \bot = \psi \rightarrow \bot$ 
    \end{itemize}
    \subsubsection*{Inductive Case 2:}Suppose $P(\phi_1), P(\phi_2)$for some formula, $\exists \psi_1 \equiv \phi_1, \psi_2 \equiv \phi_2$
    \begin{itemize}
        \item $(\phi_1 \lor \phi_2) = ((\neg (\neg \phi_1)) \lor \phi_2) = ((\neg \phi_1) \rightarrow \phi_2)$
        \item From inductive case 1, $((\neg \phi_1) \rightarrow \phi_2) = ((\phi_1 \rightarrow \bot) \rightarrow \phi_2) = ((\psi_1 \rightarrow \bot) \rightarrow \psi_2)$
    \end{itemize}
...
\begin{proof}
    Define property $P(\phi)$ to be there exists a CNF $\psi$  that is logically equivalent to $\phi$
    \subsubsection*{Base Case:} $\phi = p$.
    An atomic proposition $p$ is a clause containing a single literal. A single clause is, by definition, a CNF. Thus, $P(p)$ holds.

    \subsubsection*{Inductive Case 1 $\neg$:} Assume $P(\phi)$ holds.
    If $\phi$ has a CNF, we can first convert $\neg \phi$ into NNF \href{https://en.wikipedia.org/wiki/Negation_normal_form}{https://en.wikipedia.org/wiki/Negation\_normal\_form}
    \begin{itemize}
        \item $\neg(A \wedge B) \equiv \neg A \vee \neg B$
        \item $\neg(A \vee B) \equiv \neg A \wedge \neg B$
        \item $\neg \neg A \equiv A$
    ...
    \subsubsection*{Inductive Case 2 $\lor$:} Assume $P(\phi_1)$ and $P(\phi_2)$ hold.
    Let $\psi_1 = \bigwedge_{i} C_i$ and $\psi_2 = \bigwedge_{j} D_j$ be CNFs.
    \begin{itemize}
        \item $\phi_1 \vee \phi_2 \equiv (\bigwedge_{i} C_i) \vee (\bigwedge_{j} D_j)$.
        \item By the Distributive Law ($A \vee (B \wedge C) \equiv (A \vee B) \wedge (A \vee C)$), we distribute the $\lor$ over the $\land$:
        \item $\phi_1 \vee \phi_2 \equiv \bigwedge_{i} \bigwedge_{j} (C_i \vee D_j)$.
        \item Since the disjunction of two clauses $C_i \vee D_j$ is also a clause, the final expression is a CNF.
    \end{itemize}
\end{proof}
\section*{Q4}

\begin{definition}[Occurs]
Define $OCCUR:\mathrm{FORM}\to\mathcal{P}(P)$ inductively by:
\begin{itemize}
  \item If $\phi=p\in P$, then $OCCUR(\phi)=\{p\}$,
  \item If $\phi=\neg\psi$, then $OCCUR(\phi)=OCCUR(\psi)$,
  \item If $\phi=\psi_1\oplus\psi_2$ with $\oplus\in\{\wedge,\vee\}$, then
  $OCCUR(\phi)=OCCUR(\psi_1)\cup OCCUR(\psi_2)$.
\end{itemize}
\end{definition}

\begin{lemma}[Relevance Lemma]
Let $\phi$ be a propositional formula and let $v_1,v_2:P\to\{\mathrm{true},\mathrm{false}\}$
be valuations such that $v_1(p)=v_2(p)$ for all $p\in OCCUR(\phi)$.
Then $v_1\models\phi$ if and only if $v_2\models\phi$.
\end{lemma}

Prove this lemma using structural induction.
\begin{proof}
We define the property $P(\phi)$ as: for any $v_1, v_2$, if $v_1(p) = v_2(p)$ $\forall p \in OCCUR(\phi)$, $v_1 \models \phi \iff v_2 \models \phi$. 
\subsubsection*{Base Case:} $\phi = p$ for some $p \in P$
\begin{itemize}
    \item $OCCUR(p) = \{p\}$,  $v_1(p) = v_2(p)$
    \item $v_1 \models p \iff v_1(p) = \text{true}$ and $v_2 \models p \iff v_2(p) = \text{true}$
    \item $v_1(p) = v_2(p)$, $v_1 \models p \iff v_2 \models p$
\end{itemize}

\subsubsection*{Inductive Step 1 (negation):} Assume $P(\psi)$ holds. Let $\phi = \neg \psi$.
\begin{itemize}
    \item $OCCUR(\phi) = OCCUR(\psi)$, $v_1 \models \psi \iff v_2 \models \psi$
    \item $v_1 \models \neg \psi \iff v_1 \not\models \psi$ and $v_2 \models \neg \psi \iff v_2 \not\models \psi$
    \item Since the truth values of $\psi$ are the same, so do their negation. 
\end{itemize}
\subsubsection*{Inductive Step 2 (Binary connective):} Assume $P(\psi_1)$ and $P(\psi_2)$ hold. 
\begin{itemize}
    \item Let $\phi = \psi_1 \oplus \psi_2$ for $\oplus \in \{\land, \lor\}$
    \item $OCCUR(\phi) = OCCUR(\psi_1) \cup OCCUR(\psi_2)$, if $v_1, v_2$ agree on $OCCUR(\phi)$, they agree on both $OCCUR(\psi_1)$ and $OCCUR(\psi_2)$
    \item  $v_1 \models \psi_1 \iff v_2 \models \psi_1$ and $v_1 \models \psi_2 \iff v_2 \models \psi_2$
    \item the satisfaction of $\psi_1 \oplus \psi_2$ depends only on the truth values of $\psi_1$ and $\psi_2$, and those values are the same for $v_1$ and $v_2$, thus $v_1 \models \phi \iff v_2 \models \phi$
\end{itemize}
...
$v_1(p)=v_2(p)$ for all $p$ in some set $S\supseteq OCCUR(\phi)$.
Then $v_1\models\phi$ if and only if $v_2\models\phi$.
\end{lemma}

Prove this lemma.
\begin{proof}
Let $\phi$ be a formula and $S$ be a set such that $S \supseteq OCCUR(\phi)$.
Suppose $v_1(p) = v_2(p) \forall p \in S$.
Since $S$ contains all elements of $OCCUR(\phi)$, the condition $v_1(p) = v_2(p) \forall p \in S \implies v_1(p) = v_2(p) \forall p \in OCCUR(\phi)$.
By the Relevance lemma, if $v_1$ and $v_2$ agree on $\forall p \in OCCUR(\phi)$, then $v_1 \models \phi \iff v_2 \models \phi$.
\end{proof}
\section*{Q6}
...
Prove this lemma.
\begin{proof}
We prove the equivalence of the two statements by showing both implications.

($\Leftarrow$) Suppose $\phi_1$ and $\phi_2$ are logically equivalent. $\forall v$, $v \models \phi_1 \iff v \models \phi_2$.
Consider $v_1, v_2$ that agree on $OCCUR(\phi_1) \cup OCCUR(\phi_2)$, we have:
\begin{itemize}
    \item $v_1 \models \phi_1 \iff v_1 \models \phi_2$
    \item $v_2 \models \phi_1 \iff v_2 \models \phi_2$
\end{itemize}
Trivial

($\Rightarrow$) Suppose $\forall v_1, v_2$ agreeing on $S = OCCUR(\phi_1) \cup OCCUR(\phi_2)$, we have $v_1 \models \phi_1 \iff v_2 \models \phi_2$.
Let $v$ be any valuation. We will show that $v \models \phi_1 \iff v \models \phi_2$.
Define $v_1 = v$ and $v_2 = v$. These valuations trivially agree on $S$.
\begin{itemize}
    \item $v_1 \models \phi_1 \iff v_2 \models \phi_2$
    \item $v \models \phi_1 \iff v \models \phi_2$
    \item $\phi_1$ and $\phi_2$ are logically equivalent
\end{itemize}
...
\end{theorem}
\begin{proof}
Suppose $G = (V, E)$ has a vertex cover $V' \subseteq V$ such that $|V'| \leq k$. We construct a valuation $v$ as follows:
\begin{enumerate}
    \item For each $u \in V$, set $v(p_{u, in}) = \text{true}$ if $u \in V'$, and $v(p_{u, in}) = \text{false}$ otherwise
    \item Set $v(p_{u, out}) = \neg v(p_{u, in}) \implies \phi_{in} \land \phi_{out}$ is satisfiable
    \item Since $V'$ is a vertex cover, for every edge $\{u, v\} \in E$, at least one of $u$ or $v$ must be in $V'$. Thus, $v(p_{u, in}) \lor v(p_{v, in})$ is true for all edges, satisfying $\phi_e$.
    \item Define $v(c_{i, j})$ to be true if and only if $|\{u_1, \dots, u_i\} \cap V'| \geq j$. 
    \begin{itemize}
        \item the recurrence $[c_{i-1, j} \lor (c_{i-1, j-1} \land p_{i, in})] \leftrightarrow c_{i, j}$ is satisfied because the count of vertices in the cover up to $i$ increases only if the $i$-th vertex is in the cover.
        \item Since $|V'| \leq k$, the total count of vertices in $V'$ after considering all $|V|$ vertices cannot be $k+1$. Thus, $v(c_{|V|, k+1}) = \text{false}$, which satisfies $\neg c_{|V|, k+1}$.
    \end{itemize}
\end{enumerate}
...
\begin{proof}
    Define proposition set $P = \{p_{i, j, k} \mid i, j, k \in [1, N]\}$, representing cell $(i, j)$ is assigned the number $k$
    We need to satisfiy the following constriants:
    \subsubsection*{Cell: At most 1 value assigned}
    \[\phi_{cell, \leq 1} = \bigwedge_{i=1}^N \bigwedge_{j=1}^N \bigwedge_{1\leq k\leq m\leq N} (\neg p_{i,j,k} \lor \neg p_{i,j,m})\]
    \subsubsection*{Cell: At least 1 value assigned}
    \[\phi_{cell, \geq 1} = \bigwedge_{i=1}^N \bigwedge_{j=1}^N \bigvee_{k=1}^N p_{i,j,k}\]
    \subsubsection*{Row: At most 1 value assigned}
    \[\phi_{row, \leq 1} = \bigwedge_{i=1}^N \bigwedge_{k=1}^N \bigwedge_{1\leq j < m \leq N} (\neg p_{i,j,k} \lor \neg p_{i,m,k})\]
    \subsubsection*{Column: At most 1 value assigned}
    \[\phi_{col, \leq 1} = \bigwedge_{j=1}^N \bigwedge_{k=1}^N \bigwedge_{1\leq i < m \leq N} (\neg p_{i,j,k} \lor \neg p_{m,j,k})\]
    \subsubsection*{Block: At most 1 value assigned}
    Let $n = \sqrt{N}$. For each block starting at row $r \in \{0, \dots, n-1\}$ and column $c \in \{0, \dots, n-1\}$:
    \[\phi_{block, \leq 1} = \bigwedge_{r=0}^{n-1} \bigwedge_{c=0}^{n-1} \bigwedge_{k=1}^N \bigwedge_{(i,j) \neq (i',j')} (\neg p_{rn+i, cn+j, k} \lor \neg p_{rn+i', cn+j', k})\]
    ...
    \[\phi_{Sudoku} = \phi_{cell, \leq 1} \land \phi_{cell, \geq 1} \land \phi_{row, \leq 1} \land \phi_{col, \leq 1} \land \phi_{block, \leq 1} \land \phi_{clues}\]
\end{proof}

\subsection*{Correctness of Encoding}

\begin{theorem}
    The formula $\phi_{Sudoku}$ is satisfiable $\iff$ the Sudoku puzzle has a valid solution
\end{theorem}

\begin{proof}
    $(\Rightarrow)$ Suppose $\phi_{Sudoku}$ is satisfiable under valuation $v$. We construct a Sudoku assignment by setting cell $(i, j) = k$ if $v(p_{i,j,k}) = \text{true}$. 
    \begin{itemize}
        \item By $\phi_{cell, \geq 1}$ and $\phi_{cell, \leq 1}$, every cell is assigned exactly one value. 
        \item By $\phi_{row, \leq 1}$, no value $k$ is repeated in a row. Since there are $N$ cells in a row and $N$ possible values, each value must appear exactly once. 
        \item The same logic applies to $\phi_{col, \leq 1}$ and $\phi_{block, \leq 1}$ for columns and $n \times n$ subgrids.
        \item $\phi_{clues}$ ensures the solution respects the starting digits.
    \end{itemize}
    ...
    \begin{itemize}
        \item Because the solution is valid, each cell has one number (satisfying cell constraints).
        \item Because rows, columns, and blocks contain no duplicates, the "at most one" ($\leq 1$) constraints for each are satisfied.
        \item Because the solution contains the initial clues, $\phi_{clues}$ is satisfied.
    \end{itemize}
...
Let $\top$ and $\bot$ be the 0-place connectives (truth and falsity). For a wff $\phi$ and sentence symbol $A$, let $\phi^A_{\top}$ and $\phi^A_{\bot}$ denote the substitution of $A$ with $\top$ and $\bot$ respectively. Define the formula $\phi^*_A = (\phi^A_{\top} \lor \phi^A_{\bot})$.

\begin{proof}
\subsubsection*{1. $\phi \models \phi^*_A$.}
Let $v$ be a valuation such that $v \models \phi$.
\begin{itemize}
    \item If $v(A) = T\implies v(\phi) = v(\phi^A_{\top})$.  $v \models \phi \implies v \models \phi^A_{\top}$.
    \item If $v(A) = F\implies v(\phi) = v(\phi^A_{\bot})$. $v \models \phi \implies v \models \phi^A_{\bot}$.
    \item either way, $v \models (\phi^A_{\top} \lor \phi^A_{\bot})$ Thus, $\phi \models \phi^*_A$.
\end{itemize}

\subsubsection*{2. If $\phi \models \psi$ and $A$ does not occur in $\psi$, then $\phi^*_A \models \psi$.}
Let $v \models \phi^*_A \implies v \models \phi^A_{\top}$ or $v \models \phi^A_{\bot}$.
\begin{itemize}
    \item Case 1: $v \models \phi^A_{\top}$. Let $v'$ be valuation $v[A \mapsto \text{true}] \implies v' \models \phi$. $\phi \models \psi \implies v' \models \psi$. $A \not \in OCCUR(\psi) \implies \psi$ is independent of $A \implies v \models \psi$.
    \item Case 2: similar
    \item Thus, $\phi^*_A \models \psi$
\end{itemize}

\subsubsection*{3. $\phi$ is satisfiable iff $\phi^*_A$ is satisfiable.}
\begin{itemize}
    \item ($\Rightarrow$) $\phi$ is satisfiable $ \implies \exists v \models \phi$. By part 1, $\phi \models \phi^*_A \implies v \models \phi^*_A$
    \item ($\Leftarrow$) $\phi^*_A$ is satisfiable $\implies \exists v, v \models \phi^A_{\top}$ or $v \models \phi^A_{\bot}$. $v \models \phi^A_{\top} \implies v[A \mapsto \text{true}] \models \phi$. $v \models \phi^A_{\bot} \implies v[A \mapsto \text{false}] \models \phi$
\end{itemize}
\end{proof}


\begin{theorem}[Interpolation Theorem]
If $\alpha \models \beta$, then there exists a formula $\gamma$ such that $\alpha \models \gamma \models \beta$, and $OCCUR(\gamma) \subseteq OCCUR(\alpha) \cap OCCUR(\beta)$.
\end{theorem}

\begin{proof}
Let $S = OCCUR(\alpha) \setminus OCCUR(\beta)$,
\begin{itemize}
    \item Let $\alpha_0 = \alpha$.
    \item $\forall i \in [1, n]$, let $\alpha_i = (\alpha_{i-1})_{A_i}^*$.
    \item By part 1, $\alpha_{i-1} \models \alpha_i$, by transitivity $\alpha \models \alpha_n$.
    \item By part 2, $\alpha \models \beta$ and $A_1 \notin OCCUR(\beta)\implies \alpha_1 \models \beta$. Repeat $\forall A_i \in S, \alpha_n \models \beta$.
    \item $\gamma = \alpha_n$ contains no propositions from $S$, meaning $OCCUR(\gamma) \subseteq OCCUR(\alpha) \cap OCCUR(\beta)$.
\end{itemize}
\end{proof}
...
\end{enumerate}
\subsubsection*{Inductive steps: Suppose this holds for $t_1, t_2, \dots$}
$t' \equiv f(t_1, t_2, \dots)$: from definition, $Val_{\mathfrak{A}, \alpha_1}(f(t_1, t_2, \dots)) = I(f)(Val_{\mathfrak{A}, \alpha_1}(t_1), Val_{\mathfrak{A}, \alpha_1}(t_2)\dots)$. By IH, 
tuple $(Val_{\mathfrak{A}, \alpha_1}(t_1), Val_{\mathfrak{A}, \alpha_1}(t_2), \dots) = (Val_{\mathfrak{A}, \alpha_2}(t_1), Val_{\mathfrak{A}, \alpha_2}(t_2), \dots) \implies I_{\mathfrak{A}}(f)(Val_{\mathfrak{A}, \alpha_1}(t_1), Val_{\mathfrak{A}, \alpha_1}(t_2), \dots)
 = I_{\mathfrak{A}}(f)(Val_{\mathfrak{A}, \alpha_2}(t_1), Val_{\mathfrak{A}, \alpha_2}(t_2), \dots) \implies Val_{\mathfrak{A}, \alpha_1}(t) = Val_{\mathfrak{A}, \alpha_2}(t)$
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Not satisfiable in FOL: equality is reflexive, so $c=c$ must be true.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item Not satisfiable in FOL: equality is symmetric, so $c=d \Rightarrow d=c$.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item Not satisfiable in FOL: equality is transitive, so $c=d$ and $d=e$ imply $c=e$.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item Not satisfiable in FOL: functions preserve equality, so $c=d \Rightarrow f(c)=f(d)$.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item Not satisfiable in FOL: relations respect equality, so $c=d \Rightarrow (R(c)\leftrightarrow R(d))$.
    \end{itemize}
\end{enumerate}
\bigskip
...
\begin{proof}
    ...
    Suppose $\Gamma$ is satisfiable. Then there exists a structure
    ...
    Then $\mathfrak A'$ satisfies all equality axioms $E$, since identity is an equivalence relation and is preserved by functions and relations.
